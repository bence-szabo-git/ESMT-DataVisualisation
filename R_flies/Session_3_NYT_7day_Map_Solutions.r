# ============================================================================
# COVID-19 Seven-Day New Cases Per 100k - Comprehensive County Map
...
# SECTION 9: CHOROPLETH MAP CREATION
# ============================================================================

# Create the final choropleth visualization using ggplot2 grammar of graphics
ggplot(cnty_data) +         # Use integrated county data as base layer
  
  # Draw county polygons with color-coded fill based on case rate categories
  geom_sf(
    # Aesthetic mapping: fill color determined by rate category
    aes(fill = cut(rate_7d_per_100k,          # Convert continuous to categorical
                   breaks = breaks,            # Use defined breakpoints
                   right = FALSE)),            # Intervals: [a,b) not (a,b]
    # Example: [0,5), [5,10), [10,25), etc.
    color = NA              # No county border lines for clean appearance
    # Alternative: color = "white", size = 0.05 for subtle borders
  ) +
  
  # Define custom sequential color palette optimized for case rate visualization
  scale_fill_manual(
    name = "7-day new cases\nper 100k",      # Legend title with line break
    ...
    drop = TRUE     # Only show legend categories that exist in the data
    # drop = FALSE would show all categories even if unused
    # TRUE creates cleaner legend when some categories empty
  ) +
  
  # Set precise map coordinates to eliminate white space
  coord_sf(
    xlim = c(bb["xmin"], bb["xmax"]),   # Horizontal extent from bounding box
    ylim = c(bb["ymin"], bb["ymax"]),   # Vertical extent from bounding box
    expand = FALSE,                     # No padding around map edges
    datum = NA                          # Remove coordinate reference system grid
    # Creates clean map without lat/lon lines
  ) +
  
  # Add comprehensive labeling and source attribution
  labs(
    title = "Current COVID-19 situation (7-day new cases per 100k)",
    subtitle = paste("Latest date:", max(nyt$date, na.rm = TRUE)),  # Dynamic date
    caption = "Source: NYT (cases), ACS 2022 population; map via tigris + sf"
  ) +
  
  # Apply clean cartographic theme optimized for choropleth maps
  theme_void() +            # Remove axes, gridlines, background elements
  # Designed specifically for clean map visualization
  
  # Fine-tune visual elements for publication quality
  theme(
    legend.position = "right",          # Position legend on right side
    plot.title = element_text(
      face = "bold",                    # Bold title for emphasis
      size = 16                         # Larger font size for prominence
    ),
    plot.margin = margin(0,0,0,0)       # Remove plot margins for tight layout
    # Maximizes map area within available space
  )
